...
2. **Visual reference**: a screenshot provided directly in chat by the user, described in full in Section 3 below (not committed as a binary asset in this repo — see Section 12, Limitation 1). Confirmed to match the current production catalog page's actual markup/CSS (see Section 3's cross-check against source SCSS).
3. **Target system architecture & engineering conventions**: `Copilot.md` (repo root) — this is the authoritative source for how code in this repo should be organized and written; Section 4 below derives directly from it rather than proposing an independent structure. `to_be_react_instruction.md` (repo root) supplies the POC-specific scope/process rules layered on top.
...
**What the screenshot shows that is out of this LLD's scope**: the global header (logo, user identity, basket badge) belongs to the shared page layout / basket / identity modules, not the catalog screen itself. See Section 10.

## 4. Target System Architecture (from `Copilot.md` §3)

`Copilot.md` §3 ("Project Structure") is the architecture authority for this repo. Its rules, verbatim:

> Typical areas may include pages, features, components, hooks, services, data-access, types, utilities, constants, assets, and tests. Follow the project's existing structure and conventions rather than imposing a new structure. Co-locate related files when that improves cohesion. Place reusable cross-cutting code in shared locations only when reuse is proven. Avoid adding parallel folder patterns that conflict with established organization.

This repo's only "existing structure" so far is the `create-next-app` App Router scaffold (`src/app/`). Layering `Copilot.md`'s named categories onto it, without inventing a competing convention:

```
src/
├── app/                    # "pages" — Next.js App Router route segments
│   ├── page.tsx            # the catalog route ("/")
│   └── _components/        # catalog-page-local components (co-located, see below)
├── components/             # shared/cross-page components — stays EMPTY until a second
│                            #   screen actually needs one of these; created ahead of that
│                            #   proven need would violate the rule above
├── services/                # "services, data-access" — PublicApi client functions
├── types/                   # "types" — API/domain TypeScript types
├── hooks/                   # "hooks" — no entries needed for this screen (Section 7: no
│                            #   client-side state to manage)
├── constants/                # "constants" — e.g. ITEMS_PER_PAGE
└── lib/                      # "utilities" — small helpers, if/when one is needed
```

**Why the catalog components are co-located under `app/_components/` rather than the shared `components/`**: `Copilot.md` explicitly says shared locations are for *proven* reuse, and that new parallel folder patterns should be avoided. Today exactly one screen (the catalog) exists, so there is no second consumer to justify `components/` yet. If a future screen (basket, product detail, etc.) needs `ProductCard` or `Pagination`, promote it to `src/components/` at that point — not preemptively.

`hooks/` and `lib/` are listed for completeness per the taxonomy above but start empty: this screen's data fetching happens in a Server Component (Section 5), not a client hook, and no cross-cutting utility function is needed yet either. Per `Copilot.md` §1/§2 ("avoid unnecessary abstractions", "avoid creating abstraction layers without clear value"), empty placeholder folders should not be scaffolded speculatively — create `hooks/`/`lib/` only when a concrete need arises.

## 5. Route & Rendering Strategy

- **Route**: `src/app/page.tsx` (root `/`) — matches the AS-IS route exactly (the catalog listing *is* the home page, not `/catalog`).
...
| Component | File | Architecture category (§4) | Maps to (AS-IS) | Responsibility |
|---|---|---|---|---|
| `CatalogPage` | `src/app/page.tsx` | pages | `IndexModel.OnGet` | Reads `searchParams`, calls the service layer, composes the page |
| `CatalogFilters` | `src/app/_components/CatalogFilters.tsx` | components (co-located) | `Index.cshtml` filter `<form>` | Brand/Type `<select>`s + submit; plain `<form method="GET">` posting back to `/`, mirroring the AS-IS pattern exactly rather than introducing client-side filter state |
| `ProductGrid` | `src/app/_components/ProductGrid.tsx` | components (co-located) | `esh-catalog-items` wrapper | Lays out `ProductCard`s in the 3/2/1-column responsive grid |
| `ProductCard` | `src/app/_components/ProductCard.tsx` | components (co-located) | `_product.cshtml` | Image, name, price, "Add to Basket" button (button is presentational only here — no basket module exists yet in this repo, see Section 10) |
| `Pagination` | `src/app/_components/Pagination.tsx` | components (co-located) | `_pagination.cshtml` | Previous/Next `Link`s + the exact "Showing X of Y products - Page N - M" text; rendered twice per page, above and below the grid |
| `catalogApi` | `src/services/catalogApi.ts` | services, data-access | — (new, contract-derived) | Typed fetch functions against `PublicApi` |
| API types | `src/types/catalog.ts` | types | AS-IS LLD §10/§11 | TypeScript mirrors of `CatalogItemDto`, brand/type DTOs, list response |
| `ITEMS_PER_PAGE` | `src/constants/catalog.ts` | constants | `Web/Constants.cs` | Page size constant, mirrored not reconfigured (Section 8) |

## 7. Data Contracts (TypeScript, derived from AS-IS LLD §10–11 — not invented)
...
## 8. Filter & Pagination URL Design
...
- Page size is a constant (`ITEMS_PER_PAGE = 10`, `src/constants/catalog.ts`), mirrored from `Web/Constants.cs`, not sourced from config — same as AS-IS.
...
Exact hero image asset, font family, and header/nav styling were not part of this screen's own SCSS file and are out of scope here (Section 10).

## 10. Explicitly Out of Scope for This LLD
...
2. Global header/nav design (Section 10) is an open decision, not resolved by this LLD.
3. Exact JSON field casing for brand/type list responses and the error envelope (Section 7) should be confirmed against a live `PublicApi` instance or its Swagger doc before implementation, not assumed.
4. This document describes design intent only — no component code has been written yet. Implementation is a separate, follow-up change, and per this repo's standing rule, must land in commits that each keep `npm run build`/`npm run lint` passing (Section 5's `force-dynamic` requirement exists specifically to protect that in this route's case).
5. `src/components/`, `src/hooks/`, and `src/lib/` (Section 4) are named in the target architecture but intentionally not created until a concrete need exists — don't scaffold them speculatively when implementing.
...
- Target architecture source of truth: `Copilot.md` §3 (this repo's root) — see Section 4
- Process rules: `to_be_react_instruction.md` (this repo's root)
